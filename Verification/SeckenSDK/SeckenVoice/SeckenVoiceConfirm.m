...
@interface SeckenVoiceConfirm () <IFlyISVDelegate>
{
...
@implementation SeckenVoiceConfirm
                   ...
                   FailBlock:(void (^)(id))fail
{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        [[VoiceConfirmSDK currVoiceSDK] updateBrickParamUserName:_username paramToken:_token updateSuccess:^(id operationBrick) {
            
            if ([[operationBrick objectForKey:@"code"] intValue] == 200) {

                _auth_token = [operationBrick objectForKey:@"auth_token"];
                ...
                if ([SecKenKeychain passwordForService:kLocalAuthID account:kLocalAuthID]) {
                    ...
                    if (strAes != nil) {
                        ...
                        if (_auth_id != nil) {
                            ...
                            if ([[[SeckenBaseInfo sharedBaseInfo] getCurrentLanguage] isEqualToString:@"cn"]) {
                                ...
                                resutDic = @{@"hasVoice":@"1",
                                             @"hasRegID": @"1",
                                             @"msg":VOICE_CONFIRM_TIP_TRAIN_SECKEN_CN,
                                             @"auth_token":[operationBrick objectForKey:@"auth_token"]};
                            }else{
                        ...
                        NSString * regID = [operationBrick objectForKey:@"reg_id"];
                        if (regID != nil && ![regID isKindOfClass:[NSNull class]]) {
                            _auth_id = regID;
                            [self initIFLY];
                        }
                ...
                NSDictionary * dict = [[SeckenBaseInfo sharedBaseInfo] showStatusErrorCode:@{@"code":@"60006"}];
                fail(dict);
            }
            
        } updateFail:^(id operationBrick) {
...
-(void)longGes:(UILongPressGestureRecognizer *)pan
{
    ...
    if(pan.state==UIGestureRecognizerStateBegan)
    {
        ...
        [self startListenVoice];

        /*
         这块的代码是之前的，因为当时规定的是，按下收费。
        if (isFreeVoice) {
...
-(void)trainOrVerifyNumCode:(NSString *)sst
{
    if( [sst isEqualToString:VERIFY_SST] ){
        if( _codeArray!=nil && _codeArray.count > 0 ){
            NSString *ptString=[_codeArray objectAtIndex:0];
            [self defaultSetparam:_auth_id withpdwt: PWDT_NUM_CODE withptxt:ptString trainorverify:VERIFY_SST];
        }
    }
}
...
-(void)resultProcess:(NSDictionary *)dic
{
    ...
    if( [[dic objectForKey:@"dcs"] isEqualToString:@"success"] && [[dic objectForKey:@"score"] floatValue] >=64.){
        ...
        if ([[[SeckenBaseInfo sharedBaseInfo] getCurrentLanguage] isEqualToString:@"cn"]) {
            ...
            [self showAlertTitle:ALTER_TIP_SECKEN_CN
                             msg:VOICE_CONFIRM_TIP_VERIFY_SECKEN_CN
                          cancel:nil
                         comfirm:CONFIRM_SECKEN_CN
                             tag:0];
            
            if (_auth_token == nil || _auth_token.length == 0) {
                _auth_token = @"";
            }
            
            resutDic = @{@"hasVoice":@"1", @"hasRegID": @"1", @"msg":VOICE_CONFIRM_TIP_VERIFY_SECKEN_CN, @"auth_token":_auth_token};

            if (copySuccessBlock != nil) {
                copySuccessBlock(resutDic);
            }
        }else{
            
            [self showAlertTitle:ALTER_TIP_SECKEN_EN
                             msg:VOICE_CONFIRM_TIP_VERIFY_SECKEN_EN
                          cancel:nil
                         comfirm:CONFIRM_SECKEN_EN
                             tag:0];
            ...
            resutDic = @{@"hasVoice":@"1", @"hasRegID": @"1", @"msg":VOICE_CONFIRM_TIP_VERIFY_SECKEN_EN, @"auth_token":_auth_token};
            if (copySuccessBlock != nil) {
                copySuccessBlock(resutDic);
            }
        }
        ...
        NSDictionary * resutDic = nil;
        if ([[[SeckenBaseInfo sharedBaseInfo] getCurrentLanguage] isEqualToString:@"cn"]) {
            [self showAlertTitle:ALTER_TIP_SECKEN_CN msg:VOICE_CONFIRM_TIP_VERIFY_FAIL_SECKEN_CN cancel:nil comfirm:CONFIRM_SECKEN_CN tag:0];
            resutDic = @{@"hasVoice":@"0", @"hasRegID":@"0", @"msg":VOICE_CONFIRM_TIP_VERIFY_FAIL_SECKEN_CN};
            
        }else {
            [self showAlertTitle:ALTER_TIP_SECKEN_EN msg:VOICE_CONFIRM_TIP_VERIFY_FAIL_SECKEN_EN cancel:nil comfirm:CONFIRM_SECKEN_EN tag:0];
            resutDic = @{@"hasVoice":@"0", @"hasRegID":@"0", @"msg":VOICE_CONFIRM_TIP_VERIFY_FAIL_SECKEN_EN};
        }
        
        
        if (copyFailBlock != nil) {
            copyFailBlock(resutDic);
        }
    }
}
...
-(void)onError:(IFlySpeechError *)errorCode
{
                ...
                [[AVAudioSession sharedInstance] performSelector:@selector(requestRecordPermission:) withObject:^(BOOL granted) {
                    if (granted) {
                        dispatch_async(dispatch_get_main_queue(), ^{
                            ...
                            if ([[[SeckenBaseInfo sharedBaseInfo] getCurrentLanguage] isEqualToString:@"cn"]) {
                                alert = [[UIAlertView alloc]initWithTitle:nil message:VOICE_CONFIRM_TIP_TIME_SHORT_CN delegate:self cancelButtonTitle:VOICE_CONFIRM_TIP_AGAIN_CN otherButtonTitles:nil, nil];
                            }else{
                                alert = [[UIAlertView alloc]initWithTitle:nil message:VOICE_CONFIRM_TIP_TIME_SHORT_EN delegate:self cancelButtonTitle:VOICE_CONFIRM_TIP_AGAIN_EN otherButtonTitles:nil, nil];
                            }
                            [alert show];
                        });
                    }
    ...
    NSDictionary * resultDict = @{@"msg":@"原因如下：音频太短太小、太多噪音、音频内容与给定文本不一致(Audio is too short is too small, too much noise, audio content do not agree with the given text)",@"error":@"0"};
    if (copyFailBlock != nil) {
        copyFailBlock(resultDict);
    }
}
