...
+ (SeckenError *) errorWithCCCryptorStatus: (CCCryptorStatus) status
{
    ...
    SeckenError * result = [SeckenError errorWithDomain: kCommonCryptoErrorDomain code: status userInfo: userInfo];
...
@implementation SeckenCommonCrypto
...
- (NSData *) MD2Sum
{
    ...
    (void) CC_MD2( [self.data bytes], (CC_LONG)[self.data length], hash );
...
- (NSData *) MD4Sum
{
    ...
    (void) CC_MD4([self.data bytes], (CC_LONG)[self.data length], hash );
...
- (NSData *) MD5Sum
{
    ...
    (void) CC_MD5([self.data bytes], (CC_LONG)[self.data length], hash );
...
- (NSData *) SHA1Hash
{
    ...
    (void) CC_SHA1([self.data bytes], (CC_LONG)[self.data length], hash );
...
- (NSData *) SHA224Hash
{
    ...
    (void) CC_SHA224([self.data bytes], (CC_LONG)[self.data length], hash );
...
- (NSData *) SHA256Hash
{
    ...
    (void) CC_SHA256([self.data bytes], (CC_LONG)[self.data length], hash );
...
- (NSData *) SHA384Hash
{
    ...
    (void) CC_SHA384([self.data bytes], (CC_LONG)[self.data length], hash );
...
- (NSData *) SHA512Hash
{
    ...
    (void) CC_SHA512([self.data bytes], (CC_LONG)[self.data length], hash );
...
- (NSData *) AES256EncryptedDataUsingKey: (id) key error: (NSError **) error
{
    ...
    if ( error != NULL )
        *error = [SeckenError errorWithCCCryptorStatus: status];
...
{
    size_t bufsize = CCCryptorGetOutputLength( cryptor, (size_t)[self.data length], true );
    void * buf = malloc( bufsize );
    size_t bufused = 0;
    size_t bytesTotal = 0;
    *status = CCCryptorUpdate( cryptor,[self.data bytes], (size_t)[self.data length],
                              buf, bufsize, &bufused );
...
- (NSData *) HMACWithAlgorithm: (CCHmacAlgorithm) algorithm key: (id) key
{
    ...
    CCHmac( algorithm, [keyData bytes], [keyData length],[self.data bytes], [self.data length], buf );
