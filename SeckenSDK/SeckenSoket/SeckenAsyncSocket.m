...
@interface SeckenAsyncSocket (Private)
...
**/
@interface SeckenAsyncReadPacket : NSObject
{
  @public
...
@implementation SeckenAsyncReadPacket
...
**/
@interface SeckenAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation SeckenAsyncWritePacket
...
**/
@interface SeckenAsyncSpecialPacket : NSObject
{
  @public
...
@implementation SeckenAsyncSpecialPacket
...
@implementation SeckenAsyncSocket
...
- (float)progressOfReadReturningTag:(long *)tag bytesDone:(NSUInteger *)done total:(NSUInteger *)total
{
	...
	if (!theCurrentRead || ![theCurrentRead isKindOfClass:[SeckenAsyncReadPacket class]])
	{
		if (tag != NULL)   *tag = 0;
		if (done != NULL)  *done = 0;
...
- (float)progressOfWriteReturningTag:(long *)tag bytesDone:(NSUInteger *)done total:(NSUInteger *)total
{
	...
	if (!theCurrentWrite || ![theCurrentWrite isKindOfClass:[SeckenAsyncWritePacket class]])
	{
		if (tag != NULL)   *tag = 0;
		if (done != NULL)  *done = 0;
		if (total != NULL) *total = 0;
...
- (void)doAcceptFromSocket:(CFSocketRef)parentSocket withNewNativeSocket:(CFSocketNativeHandle)newNativeSocket
{
	if(newNativeSocket)
	{
		...
		SeckenAsyncSocket *newSocket = [[[self class] alloc] initWithDelegate:theDelegate];
...
- (void)recoverUnreadData
{
	if(theCurrentRead != nil)
	{
		...
		if(([theCurrentRead isKindOfClass:[SeckenAsyncReadPacket class]]) && (theCurrentRead->bytesDone > 0))
		{
...
- (NSError *)getSocketError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"AsyncSocketCFSocketError",
														 @"SeckenAsyncSocket", [NSBundle mainBundle],
...
- (NSString *)description
{
	...
	if (theCurrentRead == nil || [theCurrentRead isKindOfClass:[SeckenAsyncSpecialPacket class]])
	...
	if (theCurrentWrite == nil || [theCurrentWrite isKindOfClass:[SeckenAsyncSpecialPacket class]])
                        ...
                        tag:(long)tag
{
	...
	SeckenAsyncReadPacket *packet = [[SeckenAsyncReadPacket alloc] initWithData:buffer
	                                                    startOffset:offset
	                                                      maxLength:length
	                                                        timeout:timeout
	                                                     readLength:0
...
- (void)maybeDequeueRead
{
	...
	if((theCurrentRead == nil) && (theReadStream != NULL))
	{
		if([theReadQueue count] > 0)
		{
			...
			if([theCurrentRead isKindOfClass:[SeckenAsyncSpecialPacket class]])
			{
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	SeckenAsyncWritePacket *packet = [[SeckenAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)maybeDequeueWrite
{
	...
	if((theCurrentWrite == nil) && (theWriteStream != NULL))
	{
		if([theWriteQueue count] > 0)
		{
			...
			if([theCurrentWrite isKindOfClass:[SeckenAsyncSpecialPacket class]])
			{
				// Attempt to start TLS
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	SeckenAsyncSpecialPacket *packet = [[SeckenAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	{
		SeckenAsyncSpecialPacket *tlsPacket = (SeckenAsyncSpecialPacket *)theCurrentRead;
...
static void MyCFSocketCallback (CFSocketRef sref, CFSocketCallBackType type, CFDataRef inAddress, const void *pData, void *pInfo)
{
	@autoreleasepool {
		...
		SeckenAsyncSocket *theSocket = (__bridge SeckenAsyncSocket *)pInfo;
