...
//
//此类先不使用。
//由于本类是线程不安全的，所以先用GCDAsyncSocket试试

#import <Foundation/Foundation.h>

@class SeckenAsyncSocket;
@class SeckenAsyncReadPacket;
@class SeckenAsyncWritePacket;
...
**/
- (void)onSocket:(SeckenAsyncSocket *)sock didAcceptNewSocket:(SeckenAsyncSocket *)newSocket;

/**
 * Called when a new socket is spawned to handle a connection.  This method should return the run-loop of the
 * thread on which the new socket and its delegate should operate. If omitted, [NSRunLoop currentRunLoop] is used.
**/
- (NSRunLoop *)onSocket:(SeckenAsyncSocket *)sock wantsRunLoopForNewSocket:(SeckenAsyncSocket *)newSocket;
...
**/
- (BOOL)onSocketWillConnect:(SeckenAsyncSocket *)sock;
...
**/
- (void)onSocket:(SeckenAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)onSocket:(SeckenAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)onSocket:(SeckenAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)onSocket:(SeckenAsyncSocket *)sock
...
**/
- (void)onSocketDidSecure:(SeckenAsyncSocket *)sock;
...
@interface SeckenAsyncSocket : NSObject
{
	...
	SeckenAsyncReadPacket *theCurrentRead;
	NSTimer *theReadTimer;
	NSMutableData *partialReadBuffer;
	
	NSMutableArray *theWriteQueue;
	SeckenAsyncWritePacket *theCurrentWrite;
